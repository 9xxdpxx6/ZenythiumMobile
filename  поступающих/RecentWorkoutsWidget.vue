...
import { IonButton } from '@ionic/vueシュimport { useDataFetching } from '@/composables/useDataFetching';
...
const { data: workoutsData, loading } = useDataFetching(
  async () => {
    const result = await workoutsService.getAll({ limit: props.limit, page: 1 });
    return result.data || [];
  },
  { immediate: true }
);
